...
Shadowing2::Shadowing2()
{
	bind("pathlossExp1_", &pathlossExp1_);
	bind("pathlossExp2_", &pathlossExp2_);
	bind("std_db1_", &std_db1_);
	bind("std_db2_", &std_db2_);
	bind("dist0_", &dist0_);
	bind("dist1_", &dist1_);	
	bind("seed_", &seed_);
...
double Shadowing2::Pr(PacketStamp *t, PacketStamp *r, WirelessPhy *ifp)
{
	...
	double avg_db;
        if (dist > dist0_) {
        	if (dist > dist1_){
        	avg_db = -10.0 * pathlossExp1_ * log10(dist1_/dist0_) -10.0 * pathlossExp2_ * log10(dist/dist1_);
        	}
        	else{
            avg_db = -10.0 * pathlossExp1_ * log10(dist/dist0_);
        }} else {
            avg_db = 0.0;
        }
...
if (dist > dist1_) {
	powerLoss_db = avg_db + ranVar->normal(0.0, std_db2_);	
	}
else {
	powerLoss_db = avg_db + ranVar->normal(0.0, std_db1_);
}
...
int Shadowing2::command(int argc, const char* const* argv)
{
	if (argc == 7) {
...
double Shadowing2::getDist(double , double , double , double, double , double , double , double )
{
	return DBL_MAX;
}
